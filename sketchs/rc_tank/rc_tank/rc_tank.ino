    ...
    if(bUpdateFlags & STEERING_FLAG)
    {
      ...
      case GEAR_IDLE:
        if(unSteeringIn > (unSteeringCenter + RC_DEADBAND))
        {
          ...
          throttleRight = throttleLeft = map(unSteeringIn,unSteeringCenter,unSteeringMax,PWM_MIN,PWM_MAX - 100);
        }
        else if(unSteeringIn < (unSteeringCenter - RC_DEADBAND))
        {
          gDirection = DIRECTION_ROTATE_LEFT;
          // use steering to set throttle
          throttleRight = throttleLeft = map(unSteeringIn,unSteeringMin,unSteeringCenter,PWM_MAX - 100,PWM_MIN);
        }
        break;
      ...
      //Serial.println(throttleLeft, DEC);
      //Serial.println("PWM_RIGHT: ");
      //Serial.println(throttleRight, DEC);
    }
  }
